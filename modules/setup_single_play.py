...
from modules.tools import get_user_true_false, user_play_card, print_multiple_cards, get_player_at_position
...
def setup_single_play(game_dict):
    ...
    cards = cards = game_dict["players"][game_dict["turn"]]["cards"]
    ...
    if get_user_true_false(show_message, error_message, cards):
        ...
        for _ in range(2):
            game_dict["players"][game_dict["turn"]]["cards"], skat_card = user_play_card(show_message, error_message, cards, value_dict, suit_dict)
...
def get_play_type(show_message, error_message, gamemode_dict, user_cards):
    ...
    print(show_message)
    for num, game in gamemode_dict.items():
        print(f"{num}: {game}")
