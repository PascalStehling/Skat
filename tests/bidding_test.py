...
class Test_get_player_turn(unittest.TestCase):
    ...
    def test_no_passed_middlehand_made_play(self):
        ...
        self.assertEqual(bidding.get_player_turn(self.game_dict), 0) # After middlehand played, forhand needs to play next

    def test_no_passed_forhand_made_play(self):
        self.reset_game_dict(turn=0)
        self.assertEqual(bidding.get_player_turn(self.game_dict), 1) # After forhand played, middlehand needs to play next

    def test_no_passed_middlehand_passed(self):
        self.reset_game_dict(passed=[1])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After middlehand passed, backhand needs to play next

    def test_no_passed_forhand_passed(self):
        self.reset_game_dict(turn=0, passed=[0])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After forhand passed, backhand needs to play next
    ...
    def test_middlehand_passed_backhand_play(self):
        ...
        self.assertEqual(bidding.get_player_turn(self.game_dict), 0) # After middlehand passed and backhand played, forhand needs to play next

    def test_middlehand_passed_forhand_play(self):
        self.reset_game_dict(turn=0, passed=[1])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After middlehand passed and forhand played, backhand needs to play next

    def test_forhand_passed_backhand_play(self):
        self.reset_game_dict(turn=2, passed=[0])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 1) # After forhand passed and backhand played, middlehand needs to play next

    def test_forhand_passed_middlehand_play(self):
        self.reset_game_dict(turn=1, passed=[0])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After forhand passed and middlehand played, backhand needs to play next
    ...
    def test_middlehand_passed_backhand_passes(self):
        ...
        self.assertEqual(bidding.get_player_turn(self.game_dict), 0) # After middlehand and backhand passed, forhand needs to play next

    def test_middlehand_passed_forhand_passes(self):
        self.reset_game_dict(turn=0, passed=[1,0])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After middlehand and forhand passed, backhand needs to play next

    def test_forhand_passed_backhand_passes(self):
        self.reset_game_dict(turn=2, passed=[0,2])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 1) # After forhand and backhand passed, middlehand needs to play next

    def test_forhand_passed_middlehand_passes(self):
        self.reset_game_dict(turn=1, passed=[0,1])
        self.assertEqual(bidding.get_player_turn(self.game_dict), 2) # After forhand and middlehand passed, backhand needs to play next
...
class Test_check_end_bidding(unittest.TestCase):
    ...
    def test_no_one_passes(self):
        self.reset_game_dict()
        self.assertFalse(bidding.check_end_bidding(self.game_dict))
    ...
    def test_three_passes(self):
        ...
        self.assertTrue(bidding.check_end_bidding(self.game_dict))
